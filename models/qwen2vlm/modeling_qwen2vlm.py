...
from .configuration_qwen2vlm import Qwen2vlmConfig, UrsaConfig
from deepseek_vl.models.clip_encoder import CLIPVisionTower, HybridVisionTower
from models.deepseek_vl.models import VisionConfig, MlpProjector, AlignerConfig

@dataclass
class Qwen2vlmCausalLMOutputWithPast(ModelOutput):
...
class Qwen2vlmAligner(nn.Module):
    def __init__(self, config: Qwen2vlmConfig):
        super().__init__()
        vision_hidden_size = config.vision_config.hidden_size
        text_hidden_size = config.text_config.hidden_size

        self.gate_proj = nn.Linear(vision_hidden_size, text_hidden_size, bias=False)
        self.up_proj = nn.Linear(vision_hidden_size, text_hidden_size, bias=False)
        self.down_proj = nn.Linear(text_hidden_size, text_hidden_size, bias=False)
        self.act_fn = ACT2FN[config.projector_hidden_act]

    def forward(self, image_features):
        gate_output = self.gate_proj(image_features)
        activated_gate_output = self.act_fn(gate_output)
        up_output = self.up_proj(image_features)
        intermediate_output = activated_gate_output * up_output
        output = self.down_proj(intermediate_output)
        return output


class Qwen2vlmPreTrainedModel(PreTrainedModel):
    config_class = Qwen2vlmConfig
...
class Qwen2vlmForConditionalGeneration(Qwen2vlmPreTrainedModel):
    def __init__(self, config: Qwen2vlmConfig):
        super().__init__(config)
        self.vision_model = AutoModel.from_config(config.vision_config)
        self.aligner = Qwen2vlmAligner(config)
    ...
    def forward(
    ...
    ) -> Union[Tuple, Qwen2vlmCausalLMOutputWithPast]:
        ...
        return Qwen2vlmCausalLMOutputWithPast(
            loss=loss,
            logits=logits,
